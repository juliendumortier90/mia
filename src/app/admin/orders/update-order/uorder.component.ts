...
export class AdminUpdateOrderComponent implements OnInit {
  ...
  changeOrderStatus(status: string) {
    this.http.post('https://zq3s7ojolk.execute-api.eu-west-1.amazonaws.com/prod/admin/update-order-status', { id: this.order.id, status: status})
        .subscribe((data: any) => {
  ...
  changeOrderComment(comment: string) {
    this.http.post('https://zq3s7ojolk.execute-api.eu-west-1.amazonaws.com/prod/admin/update-order-comment', { id: this.order.id, comment: comment} )
        .subscribe((data: any) => {
